Public Module ModWatcher
    ...
    Public Class Watcher
        ...
        Private QuickPlayWorld As String = ""
        Private QuickPlayAutoCloseTriggered As Boolean = False
        Private QuickPlayPlayerLeft As Boolean = False
        Public Sub New(Loader As LoaderTask(Of Process, Integer), Instance As McInstance, WindowTitle As String)
            ...
            QuickPlayWorld = If(ModLaunch.CurrentLaunchOptions?.QuickPlaySingleplayer, "")
            WatcherLog("开始 Minecraft 日志监控")
            If Me.WindowTitle <> "" Then WatcherLog("要求窗口标题：" & WindowTitle)
            If QuickPlayWorld.Length > 0 Then WatcherLog("已启用快捷启动世界的自动退出监控")
        ...
        Private Sub GameLog(Text As String)
            ...
            If Not QuickPlayAutoCloseTriggered AndAlso QuickPlayWorld.Length > 0 AndAlso State = MinecraftState.Running AndAlso Not GameProcess.HasExited Then
                If Text.ContainsF("ThreadedAnvilChunkStorage", True) AndAlso Text.ContainsF("All chunks are saved", True) AndAlso QuickPlayPlayerLeft Then
                    QuickPlayAutoCloseTriggered = True
                    WatcherLog("检测到世界保存完成，开始关闭 Minecraft")
                    RunInNewThread(AddressOf QuickPlayAutoClose, "Minecraft QuickPlay AutoClose PID " & PID)
                End If
            End If
            '检测玩家退出（用于区分启动时和退出时的 chunks saved）
            If QuickPlayWorld.Length > 0 AndAlso State = MinecraftState.Running Then
                If Text.ContainsF("has left the game", True) OrElse Text.ContainsF("lost connection", True) OrElse Text.ContainsF("disconnected", True) Then
                    QuickPlayPlayerLeft = True
                End If
            End If
            '进度处理
            ...
            End If
        End Sub
        Private Sub QuickPlayAutoClose()
            Try
                If GameProcess Is Nothing OrElse GameProcess.HasExited Then Return
                Dim Handle As IntPtr = WindowHandle
                If Handle = IntPtr.Zero Then
                    Dim Window = TryGetMinecraftWindow()
                    If Window IsNot Nothing Then Handle = Window.Value.Key
                End If
                If Handle <> IntPtr.Zero Then
                    PostMessage(Handle, &H10UI, 0, 0)
                Else
            ...
            Catch ex As Exception
                Log(ex, "快捷启动世界自动关闭失败", LogLevel.Feedback)
                Try
                    If GameProcess IsNot Nothing AndAlso Not GameProcess.HasExited Then GameProcess.Kill()
                Catch
                End Try
            End Try
        End Sub
        ...
        Private Sub TimerWindow()
            Try
                ...
                If Not MinecraftWindowName.StartsWithF("FML") AndAlso Not MinecraftWindowName.StartsWithF("Quilt Loader") Then
                    ...
                    IsWindowFinished = True
                    ' #region debug-point A:window-found
                    DebugReportEvent("A", "ModWatcher.TimerWindow:330", "minecraft window found", "{""windowHwnd"":" & MinecraftWindowHandle.ToInt64 & ",""foregroundHwnd"":" & DebugForegroundWindowHwnd() & ",""launchVisibleMode"":" & Setup.Get("LaunchArgumentVisible") & ",""windowType"":" & Setup.Get("LaunchArgumentWindowType") & "}")
                    ' #endregion
                    '如果启动器仍保持可见或只是最小化，主动把游戏切到前台，
                    '避免焦点还停留在启动器上，导致 GLFW 的光标状态异常。
                    If Setup.Get("LaunchArgumentVisible") >= 4 Then
                        RunInNewThread(
                        Sub()
                            Try
                                Thread.Sleep(300)
                                ' #region debug-point A:before-focus-switch
                                DebugReportEvent("A", "ModWatcher.TimerWindow:339", "before focus switch", "{""windowHwnd"":" & WindowHandle.ToInt64 & ",""foregroundHwnd"":" & DebugForegroundWindowHwnd() & ",""launchVisibleMode"":" & Setup.Get("LaunchArgumentVisible") & "}")
                                ' #endregion
                                ShowWindowToTop(WindowHandle)
                                WatcherLog($"已切换到 Minecraft 窗口：{MinecraftWindowHandle.ToInt64}")
                                ' #region debug-point A:after-focus-switch
                                DebugReportEvent("A", "ModWatcher.TimerWindow:343", "after focus switch", "{""windowHwnd"":" & WindowHandle.ToInt64 & ",""foregroundHwnd"":" & DebugForegroundWindowHwnd() & ",""launchVisibleMode"":" & Setup.Get("LaunchArgumentVisible") & "}")
                                ' #endregion
                                RunInNewThread(
                                Sub()
                                    Try
                                        ...
                                        If State = MinecraftState.Running AndAlso Not GameProcess.HasExited Then
                                            ' #region debug-point E:delayed-cursor-recover
                                            DebugReportEvent("E", "ModWatcher.TimerWindow:350", "before delayed ensure-cursor", "{""windowHwnd"":" & WindowHandle.ToInt64 & ",""foregroundHwnd"":" & DebugForegroundWindowHwnd() & ",""launchVisibleMode"":" & Setup.Get("LaunchArgumentVisible") & "}")
                                            ' #endregion
                                            EnsureCursorVisible()
                                        End If
                                    Catch ex As Exception
                                        Log(ex, "延后恢复 Minecraft 光标失败")
                                    End Try
                                End Sub, "MinecraftCursorRecover")
                            Catch ex As Exception
                                Log(ex, "切换 Minecraft 窗口到前台失败")
                            End Try
                        End Sub, "MinecraftWindowFocus")
                    End If
                    '最大化
