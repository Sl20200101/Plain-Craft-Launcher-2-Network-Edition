...
Done:
    ...
    Private Sub AnalyzeCrit1()
        ...
        '游戏日志分析
        If LogMc IsNot Nothing Then
            If LogMc.Contains("Unrecognized option: --sun-misc-unsafe-memory-access=allow") Then
                AppendReason(CrashReason.Java版本过低)
            ElseIf LogMc.Contains("Unrecognized option:") Then
                AppendReason(CrashReason.Java虚拟机参数有误)
            End If
            ...
            If LogMc.Contains("Unsupported class file major version") Then
                Dim VersionNeededMajor = RegexSeek(LogMc, "(?<=Unsupported class file major version )\d+")
                If VersionNeededMajor IsNot Nothing Then
                    AppendReason(CrashReason.Java版本过低, (CInt(VersionNeededMajor) - 44).ToString)
                Else
            ...
            If LogMc.Contains("has been compiled by a more recent version of the Java Runtime (class file version ") Then
                Dim VersionNeededMajor = RegexSeek(LogMc, "(?<=class file version )\d+")
                If VersionNeededMajor IsNot Nothing Then
                    AppendReason(CrashReason.Java版本过低, (CInt(VersionNeededMajor) - 44).ToString)
                Else
                    AppendReason(CrashReason.Java版本过低)
            ...
            If LogMc.Contains("Unsupported major.minor version") Then
                Dim VersionNeededMajor = RegexSeek(LogMc, "(?<=Unsupported major.minor version )\d+")
                If VersionNeededMajor IsNot Nothing Then
                    AppendReason(CrashReason.Java版本过低, (CInt(VersionNeededMajor) - 44).ToString)
                Else
                    AppendReason(CrashReason.Java版本不兼容)
    ...
    Private Function GetAnalyzeResult(IsHandAnalyze As Boolean) As String
        ...
        For Each Reason In CrashReasons
            ...
            Select Case Reason.Key
                ...
                Case CrashReason.Java版本过低
                    If Additional.Any Then
                        Results.Add("当前JAVA版本过低（需要 Java " & Additional.First & "），请在设置→游戏JAVA中选择自动选择适合的JAVA或者选择更高版本的JAVA")
                    Else
                        Results.Add("当前JAVA版本过低，请在设置→游戏JAVA中选择自动选择适合的JAVA或者选择更高版本的JAVA")
                    End If
