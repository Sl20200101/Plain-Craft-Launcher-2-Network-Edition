...
#Region "开始"
    ...
    Public Function DebugForegroundWindowHwnd() As Long
        Try
        ...
        End Try
    End Function
    Public Sub DebugReportEvent(HypothesisId As String, Location As String, Message As String, Optional Data As String = "{}")
        Try
            Log("[DEBUG-CURSOR] " & HypothesisId & " " & Location & " " & Message & " " & Data, LogLevel.Developer)
            Dim EnvPath As String = Path & ".dbg\game-cursor-hidden.env"
            Dim Url As String = "http://127.0.0.1:7777/event"
            Dim SessionId As String = "game-cursor-hidden"
            If System.IO.File.Exists(EnvPath) Then
                For Each Line In System.IO.File.ReadAllLines(EnvPath)
                    If Line.StartsWith("DEBUG_SERVER_URL=") Then Url = Line.Substring("DEBUG_SERVER_URL=".Length)
                    If Line.StartsWith("DEBUG_SESSION_ID=") Then SessionId = Line.Substring("DEBUG_SESSION_ID=".Length)
                Next
            End If
            Dim Body As String =
                "{""sessionId"":""" & SessionId & """,""runId"":""pre-fix"",""hypothesisId"":""" & HypothesisId &
                """,""location"":""" & Location & """,""msg"":""[DEBUG] " & Message &
                """,""data"":" & Data & ",""ts"":" & CLng((Date.Now.ToUniversalTime - New Date(1970, 1, 1)).TotalMilliseconds) & "}"
            Dim Request = CType(System.Net.WebRequest.Create(Url), System.Net.HttpWebRequest)
            Dim Bytes = System.Text.Encoding.UTF8.GetBytes(Body)
            Request.Method = "POST"
    ...
    End Sub
    ' #endregion
    Public Sub EnsureCursorVisible()
        Try
            ' 强制显示鼠标，直到计数为正
            Dim cursorCount As Integer = 0
            ' #region debug-point B:ensure-cursor-enter
            DebugReportEvent("B", "ModLaunch.EnsureCursorVisible:10", "enter ensure-cursor", "{""foregroundHwnd"":" & GetForegroundWindow.ToInt64 & ",""hasRunningMinecraft"":" & LCase(HasRunningMinecraft.ToString) & "}")
            ' #endregion
            For i As Integer = 0 To 63 ' 增加循环次数确保成功
                ...
                End If
            Next
            Log("[Cursor] 确保鼠标可见，最终计数: " & cursorCount, LogLevel.Developer)
            ' #region debug-point B:ensure-cursor-exit
            DebugReportEvent("B", "ModLaunch.EnsureCursorVisible:26", "exit ensure-cursor", "{""finalCursorCount"":" & cursorCount & ",""foregroundHwnd"":" & GetForegroundWindow.ToInt64 & ",""hasRunningMinecraft"":" & LCase(HasRunningMinecraft.ToString) & "}")
            ' #endregion
        Catch ex As Exception
            Log(ex, "重置鼠标显示状态失败", LogLevel.Developer)
        End Try
    End Sub
    Public Class McLaunchOptions
        ...
        ''' <summary>
        ''' QuickPlay 单人存档名（非 JVM 参数，将追加到游戏参数末尾）。
        ''' </summary>
        Public QuickPlaySingleplayer As String = Nothing
    End Class
    ''' <summary>
...
#Region "启动参数"
    ...
    Private Sub McLaunchArgumentMain(Loader As LoaderTask(Of String, List(Of McLibToken)))
        ...
        '高版本 Java 参数兼容（#7073）
        If McLaunchJavaSelected.MajorVersion < 23 Then
            Arguments = Arguments.Replace(" --sun-misc-unsafe-memory-access=allow", "").Replace("--sun-misc-unsafe-memory-access=allow ", "")
        End If
        '由 Option 传入的额外参数
        ...
        End If
        'QuickPlay 单人
        Dim World As String = If(CurrentLaunchOptions.QuickPlaySingleplayer, "")
        If World.Length > 0 Then
            FinalArguments = FinalArguments.RegexReplace("(\s--quickPlaySingleplayer\s+(""[^""]*""|\S+))", "", RegularExpressions.RegexOptions.IgnoreCase).Trim()
            If McInstanceSelected.Version.VanillaName.StartsWithF("1.") Then
                If McInstanceSelected.Version.Vanilla.Major >= 20 Then
                    FinalArguments += $" --quickPlaySingleplayer ""{World.Replace("""", "")}"""
                End If
            ElseIf RegexCheck(McInstanceSelected.Version.VanillaName, "^[2-9][0-9]*\.") Then
                FinalArguments += $" --quickPlaySingleplayer ""{World.Replace("""", "")}"""
            End If
        End If
        '输出
...
IgnoreCustomSkin:
    ...
    Private Sub McLaunchEnd()
        ...
        ' #region debug-point D:launch-end-enter
        DebugReportEvent("D", "ModLaunch.McLaunchEnd:2403", "enter launch-end", "{""launchVisibleMode"":" & Setup.Get("LaunchArgumentVisible") & ",""foregroundHwnd"":" & GetForegroundWindow.ToInt64 & "}")
        ' #endregion
        ...
        ' #region debug-point D:launch-end-before-visibility
        DebugReportEvent("D", "ModLaunch.McLaunchEnd:2417", "before launch visibility change", "{""launchVisibleMode"":" & Setup.Get("LaunchArgumentVisible") & ",""foregroundHwnd"":" & GetForegroundWindow.ToInt64 & "}")
        ' #endregion
