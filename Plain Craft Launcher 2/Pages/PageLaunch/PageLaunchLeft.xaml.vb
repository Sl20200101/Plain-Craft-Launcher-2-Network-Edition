    ...
    Public Sub PageLaunchLeft_Loaded() Handles Me.Loaded
        ...
        Sub()
            ...
            Sub()
                ...
                FrmLaunchRight.PageOnEnter()
                If McLoginAble() = "" Then McLoginLoader.Start()
            End Sub)
...
#End Region
    ...
    Private Sub BtnVersion_Click(sender As Object, e As EventArgs) Handles BtnVersion.Click
        ...
        If Not IsLoadFinished Then Return
        If Not McInstanceListLoader.State = LoadState.Finished Then LoaderFolderRun(McInstanceListLoader, McFolderSelected, LoaderFolderRunType.ForceRun, MaxDepth:=1, ExtraPath:="versions\", WaitForExit:=True)
        If Not McInstanceList.Any(Function(v) v.Key <> McInstanceCardType.Error) Then
            Hint("没有游戏版本", HintType.Red)
            Return
        End If
    ...
    Public Sub LaunchButtonClick() Handles BtnLaunch.Click
        ...
        ' #region debug-point F:launch-click
        DebugReportEvent("F", "PageLaunchLeft.LaunchButtonClick:606", "launch button click", "{""leftButtonPressed"":" & LCase((Mouse.LeftButton = MouseButtonState.Pressed).ToString) & ",""launchVisibleMode"":" & Setup.Get("LaunchArgumentVisible") & "}")
        RunInNewThread(
        Sub()
            Try
                Thread.Sleep(120)
                DebugReportEvent("F", "PageLaunchLeft.LaunchButtonClick:610", "launch click +120ms", "{""leftButtonPressed"":" & LCase((Mouse.LeftButton = MouseButtonState.Pressed).ToString) & ",""launchLoaderState"":" & CInt(McLaunchLoader.State) & "}")
            Catch
            End Try
        End Sub, "LaunchClickDebug")
        ' #endregion
        '愚人节处理
