...
#Region "窗体事件"
    ...
    Private Sub FormMain_Activated() Handles Me.Activated
        ' #region debug-point C:launcher-activated
        If HasRunningMinecraft Then DebugReportEvent("C", "FormMain.Activated:828", "launcher activated while minecraft running", "{""hidden"":" & LCase(Hidden.ToString) & ",""windowState"":" & CInt(WindowState) & ",""foregroundHwnd"":" & DebugForegroundWindowHwnd() & "}")
        ' #endregion
        Try
    ...
    Public Sub ShowWindowToTop()
        ...
        Sub()
            ...
            ' #region debug-point C:launcher-show-to-top-before
            If HasRunningMinecraft Then DebugReportEvent("C", "FormMain.ShowWindowToTop:1099", "launcher show-to-top before", "{""hidden"":" & LCase(Hidden.ToString) & ",""windowState"":" & CInt(WindowState) & ",""foregroundHwnd"":" & DebugForegroundWindowHwnd() & "}")
            ' #endregion
            ...
            Focus()
            ' #region debug-point C:launcher-show-to-top-after
            If HasRunningMinecraft Then DebugReportEvent("C", "FormMain.ShowWindowToTop:1108", "launcher show-to-top after", "{""hidden"":" & LCase(Hidden.ToString) & ",""windowState"":" & CInt(WindowState) & ",""foregroundHwnd"":" & DebugForegroundWindowHwnd() & "}")
            ' #endregion
...
#Region "切换页面"
    ...
    Private Sub BtnTitleSelect_Click(sender As MyRadioButton, raiseByMouse As Boolean) Handles BtnTitleSelect0.Check, BtnTitleSelect1.Check, BtnTitleSelect2.Check, BtnTitleSelect3.Check, BtnTitleSelect4.Check
        ...
        '如果是联机页面（Tag=2），阻止切换
        If Val(sender.Tag) = 2 Then
            '如果已经在联机按钮被触发了，重新切换回启动页面
            If PageCurrent <> PageType.Launch Then
                IsChangingPage = True
                BtnTitleSelect0.SetChecked(True, True, False)
                IsChangingPage = False
            End If
            Return
        End If
    ...
    Private Sub PageChangeActual(Stack As PageStackData, Optional SubType As PageSubType = -1)
        '如果是联机页面，强制替换为启动页面
        If Stack.Page = PageType.Link Then
            Stack = PageType.Launch
        End If
